...
public class TriageController {
    ...
    @GetMapping("/triage/exists/{idNumber}")
...
//            response.put("patient", mapPatientToDTO(patient));
    ...
    public ResponseEntity<Map<String, Object>> sendToTriage(
        ...
        try {
//            Object triageAssignment = patientService.assignToTriage(patientId, nurseId);
            response.put("success", true);
            response.put("message", "Patient successfully sent to triage.");
//            response.put("triageDetails", mapTriageToDTO(triageAssignment));
            return ResponseEntity.ok(response);
        } catch (Exception e) {
